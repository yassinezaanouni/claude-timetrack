...
struct ProjectListView: View {
    ...
    var onScrollMetricsChange: ((ScrollMetrics) -> Void)? = nil

    var body: some View {
        let projects = state.visibleProjects()
        let total = state.totalSeconds(for: state.selectedRange)
        let overlapMode = state.mergeOverlaps && state.trackingSource == .claude
        ...
        Group {
            if projects.isEmpty {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    Group {
                        if overlapMode {
                            OverlapBreakdownView(breakdown: state.overlapBreakdown())
                        } else {
